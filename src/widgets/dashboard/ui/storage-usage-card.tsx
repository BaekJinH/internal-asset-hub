...
export function StorageUsageCard({ status }: StorageUsageCardProps) {
  ...
  return (
    <Card className="overflow-hidden p-0 shadow-sm">
      ...
      <CardContent className={cn('space-y-6', dashboardCardContentClassName)}>
        <div className="flex items-end justify-between gap-stack">
          <div className="min-w-0 space-y-header-gap">
        ...
        <div className="grid grid-cols-2 gap-stack text-sm">
          <div className="rounded-lg border border-border/60 bg-muted/40 px-4 py-3">
            <p className="whitespace-nowrap text-xs text-muted-foreground">사용 가능</p>
            <p className="mt-1.5 whitespace-nowrap font-medium tabular-nums">
              {(status.storageTotal - status.storageUsed).toFixed(1)}TB
            </p>
          </div>
          <div className="rounded-lg border border-border/60 bg-muted/40 px-4 py-3">
            <p className="whitespace-nowrap text-xs text-muted-foreground">미분류 파일</p>
            <p className="mt-1.5 whitespace-nowrap font-medium tabular-nums">{status.unclassifiedFileCount}개</p>
          </div>
        </div>
      </CardContent>
    </Card>
